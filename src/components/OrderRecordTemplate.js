    ...
    return (
         this.props.data !== undefined ? this.props.data.map(el=>
            ...
            <div className='order-record' key={el.data.id}>
                ...
                <p>{"Tiempo en cocina : "+new Date(el.data.time_ready - el.data.time).getMinutes()+" minutos" }</p>
                <p>{"Entregado a las : "+new Date(el.data.time_ready).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}</p>
